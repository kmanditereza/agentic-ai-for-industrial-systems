...
from shared.models.agent import AgentCard  # AgentCard is a Pydantic model representing an agent's metadata
...
class DiscoveryClient:
    ...
    Attributes:
        registry_file (str): Path to the JSON file listing base URLs.
        base_urls (List[str]): Loaded list of agent base URLs (strings).
    """
    ...
    def __init__(self, registry_file: str = None):
        ...
        Args:
            registry_file (str, optional): Path to the registry JSON. If None,
                defaults to 'agent_registry.json' in this same directory.
        """
        if registry_file:
            self.registry_file = registry_file
        else:
            # Since discovery.py and agent_registry.json are in the same folder
